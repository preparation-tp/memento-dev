...
<Tabs>
  <TabItem value="js" label="JavaScript">
    ```tsx title="App.jsx" showLineNumbers
...
<Tabs>
  <TabItem value="js" label="JavaScript">
    ```tsx title="ThemeContext.jsx" showLineNumbers
...
Pas mal, non ? 😉  
Fini l'arborscence de composants à rallonge, on peut maintenant récupérer les données du contexte directement dans nos composants !
...
<Admonition type="example" title="Exemple de context hell">  
  ```jsx title="index.tsx" showLineNumbers
  root.render(
    <StrictMode>
      <UserProvider>
        <ThemeProvider>
...
```tsx title="Providers.tsx" {18-26} showLineNumbers
import type { ReactNode } from 'react';
...
```tsx title="index.tsx" {3-16} showLineNumbers
root.render(
  <StrictMode>
    <Providers
      providers={[
