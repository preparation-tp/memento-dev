...
Et heureusement, une n'attend pas de toi de maîtriser un serveur dans les détails, mais d'expliquer **comment** mettre en ligne ton projet.
...
??? info "Parenthèse éco-conception"
...
Maintenant, le meilleur du meilleur : les serveurs dédiés et les <abbr title="Virtual Private Server">VPS</abbr> !  
...
- Un système d'exploitation headless _(sans interface graphique)_ comme Ubuntu Server, CentOS, Debian, etc.
...
??? info "Dernière parenthèse éco-conception"
    ...
    !!! quote "Le problème se situe souvent entre la chaise et le clavier"

    Si tu recherches un hébergeur qui se veut éco-responsable _(bien plus que la moyenne)_ : [Infomaniak](https://www.infomaniak.com/fr) est un excellent choix.  
    ...
    D'ailleurs, sur toute la partie <abbr title="Réglementation Générale sur la Protection des Données">RGPD</abbr> : Infomaniak a une politique de confidentialité et de sécurité très sérieuse que tu peux retrouver [juste ici](https://www.infomaniak.com/fr/cgv/reglement-general-protection-donnees).  
...
Avant d'arrêter de parler de serveurs à configurer soi-même, je me permets d'ouvrir une toute petite rubrique sur la mise en ligne d'applications tournant sur des ports autres que le 80 _(ou 443 pour le HTTPS)_, comme on peut le faire avec un serveur Node.js.
...
??? example "Configuration Nginx pour un reverse proxy"
    ```nginx
...
### 📦 Apache
Apache est un serveur web qui est souvent utilisé pour les sites web classiques, mais qui peut également être utilisé comme reverse proxy.  
