...
<Admonition type="tip" title="Mon site est fait avec React/Angular/Vue.js, donc je valide automatiquement cette CP ?" icon="🤔">
Pas si vite ! 😏  
...
<Admonition type="info" title="Référencement d'un site généré côté client _(comme React, Vue.js etc)_">
...
<Admonition type="tip" icon="🤔" title="jQuery, c'est quoi ?">
...
<Admonition type="tip" icon="😱" title="Mais comment je vais faire pour mes consommations d'API, vu que j'utilisais `jQuery.ajax()` ?!">
...
<Admonition type="tip" icon="🤔" title="Mais ça fait beaucoup de code juste pour charger des iframes, c'est vraiment nécessaire ?">
