...
>
  J'aurai aimé te dire que oui, mais ça va être un poil trop léger pour cette
  compétence... Mais garde sous la main ta base de données non relationnelles
...
- <u class="underline">Sous-ligné</u> : Clé primaire _(en gras également car unique,
  voir ci-dessous)_
...
Si demain la norme change et que le `VARCHAR` par défaut passe à 100 caractères, tu risques de te retrouver avec des données tronquées !
